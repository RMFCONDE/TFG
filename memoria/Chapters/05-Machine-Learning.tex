...
el estimador óptimo (insesgado y de mínima varianza). Sin embargo, existe
...
la maldición de la dimensionalidad temporal: en lugar de estimar una media
...
\begin{equation}
    ...
    \sum_{t=1}^{T} (y_t - \mathbf{x}_t^T \boldsymbol{\beta})^2
...
hiperparámetro $\alpha$ ---de $0$ (MCO) a $\infty$ (todos los coeficientes
nulos)--- se selecciona por validación cruzada temporal
...
Capítulo~\ref{ch:metodologia} detalla el protocolo completo y las salvaguardas
contra el look-ahead.
...
Es importante señalar que, en finanzas, incluso un $R^2$ pequeño
...
Conviene subrayar que el objetivo no es minimizar el RMSE de
