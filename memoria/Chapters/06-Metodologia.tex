...
\begin{itemize}
    ...
    \item \texttt{optimization/}: optimización QP (\texttt{cvxpy}),
          estimación de covarianza (\texttt{scikit-}\allowbreak\texttt{learn})
          y restricciones.
...
La carpeta \texttt{tests/} valida la implementación (coincidencia del QP con la
solución analítica y condiciones KKT, Capítulo~\ref{ch:formulacion}), y
\texttt{demo/} contiene una demostración web interactiva del optimizador, como
apoyo a la presentación del trabajo.

La reproducibilidad está garantizada mediante una semilla fija
(\texttt{seed = 42}) para todos los componentes aleatorios, las versiones de las
dependencias congeladas en \texttt{requirements.txt}, datos descargables
automáticamente desde Yahoo Finance y scripts en \texttt{scripts/} que generan
todas las tablas y figuras de la memoria.
