...
seleccionadas para cubrir los principales sectores económicos con al menos
dos representantes por sector. La Tabla~\ref{tab:universo} detalla los
tickers y sus sectores GICS.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{ll}
        ...
        Consumo discrecional & HD, MCD, NKE, DIS \\
...
El benchmark de mercado es el \textbf{SPY} (SPDR S\&P 500 ETF Trust), que
replica el S\&P 500 y no forma parte del universo a optimizar.
...
(ecuación~\ref{eq:qp-riesgo}) barriendo el target $\mu^*$ y se toma el punto de
...
\[
\underbrace{r_1, \dots, r_{36}}_{\text{Train}_1} \to
\underbrace{r_{37}}_{\text{Pred}_1} \quad
\underbrace{r_2, \dots, r_{37}}_{\text{Train}_2} \to
\underbrace{r_{38}}_{\text{Pred}_2} \quad
\cdots \quad
...
\begin{itemize}
    ...
    \item \textbf{Número efectivo de activos:} media del número de activos
          con peso $> 0{,}1\%$ en cada rebalanceo.
\end{itemize}
...
El diseño experimental combina 6 carteras optimizadas (3 estimadores de
$\boldsymbol{\Sigma}$ $\times$ 2 enfoques de $\boldsymbol{\mu}$) y 2 baselines
pasivos, un backtesting walk-forward de 143 meses out-of-sample (ventanas de 36
